...
public class ApiResultActionFilter : Attribute, IAsyncActionFilter, IOrderedFilter
{
    ...
    private readonly IAPIResultsProvider _resultProvider;

    public ApiResultActionFilter(IAPIResultsProvider resultProvider)
    {
    ...
    public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
    {
        ...
        {
            // 如果是结果异常
            actionContext.Result = _resultProvider.ProcessAPIResultsException(resultException);
        ...
        else
        {
            var ApiResult = new ApiResult(StatusCodes.Status500InternalServerError, actionContext.Exception.Message,
                "");
            // 如果是结果异常
            actionContext.Result = _resultProvider.ProcessAPIResultsException(new ApiResultException(ApiResult));
        }
    }
